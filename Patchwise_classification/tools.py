### This bit of code is imported from git: https://github.com/kevin28520/My-TensorFlow-tutorials
...
# %%
def FC_layer(layer_name, x, out_nodes, keep_prob):
...
def output_Layer(layer_name, x, out_nodes):
    shape = x.get_shape()
    if len(shape) == 4:
        size = shape[1].value * shape[2].value * shape[3].value
    else:
...
def loss(logits, labels):
    ...
    with tf.name_scope('loss') as scope:
        ...
        # rmse_loss = tf.reduce_mean(tf.reduce_sum(tf.square(logits - labels), reduction_indices=[1]), name="RMSE")
...
def optimize(loss, learning_rate, global_step):
    ...
    with tf.name_scope('optimizer'):
        ...
        optimizer = tf.train.MomentumOptimizer(learning_rate=learning_rate, momentum=0.2)
...
# %%
def test_load():
    data_path = './/vgg16_weights//vgg16.npy'
